...
def producer(pid, empty, non_empty,almacen):
    new_product = 0
    ...
    #Cuando ya no hay más producciones en la poscición en la que iría el nuevo
    #producto se añade un -1 y así no se vuelve a consultar ese almacén
    almacen[i_esc] = -1    
    empty.acquire()  #Espera a que el almacén esté vacío
...
def consumer(empty, non_empty, almacenes):
    ...
    #Comprobamos que alguna de los elementos del almacen de la posicion de lec sea
    #distinto de -1
    leer =[]
    for i in range(NPROD):
    ...
    while leer != lista:
        ...
        i_lec[a] = (i_lec[a] + 1) % M #Incrementamos el índice de lectura para el almacen del 
...
def merge():
    ...
    prodlst = [ Process(target=producer,
                        name=f'prod_{pid}',
                        args=(pid, empty[pid], non_empty[pid],almacenes[pid]))
                for pid in range(NPROD) ]
    
    conslst = [ Process(target=consumer,
                      name=f"cons",
                      args=(empty, non_empty,almacenes))]
